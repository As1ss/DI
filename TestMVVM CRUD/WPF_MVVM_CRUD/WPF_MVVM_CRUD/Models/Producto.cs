...
namespace WPF_MVVM_CRUD.Models
{
    public class ProductoCollection : ObservableCollection<Producto>
    {
        ...
        protected ProductoCollection ObtenerListaProductos()
        {
            ProductoCollection listaProductos = new ProductoCollection();
            MySqlConnection conexionDB;

            try
            {
                // Obtener una conexión abierta a la BD
                conexionDB = Conexion.obtenerConexionAbierta();
                ...
                else
                {
                    try
                    {
                        ...
                        using var comando = new MySqlCommand(consulta, conexionDB);
                        comando.Prepare();
